...
function sostavChisla(massivChisel, chislo) {
  ...
  function main(array, number, checkArray) {
    let sum = 0;

    checkArray = checkArray || [];

    sum = checkArray.reduce((a, b) => {
      return a + b;
    }, 0);

    if (sum === number) {
      finalArray.push(checkArray);
    }

    // condition to skip recursive
    if (sum >= number) {
      return;
    }
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      // recursive
      main(slicedArray, number, checkArray.concat([element]));
    }
  }
  main(massivChisel, chislo);
  return finalArray;
}
